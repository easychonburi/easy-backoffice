...
async function clockIn(body,user){
  ...
  return {success:true,record_id:ref.id,branch_name:near.name,clock_in,late_min};
}
async function clockOut(body,user){
  ...
  return {success:true,clock_out,hours_worked,ot_mins,has_ot,shift_end};
}
...
async function saveStock(body,user){
  ...
  return {success:true,log_id,workOrders};
}
exports.actions=new Set(['getStaff','getBranches','getSettings','clockIn','clockOut','getTodayStatus','getTimesheets','upsertTimesheet','updateTimesheetOT','getPayrollRuns','savePayrollRun','markAdvancesDeducted','sendPayrollNotify','getAdvances','saveAdvance','getLeaves','saveLeave','deleteLeave','getStockItems','saveStockLog','getTodayStockOrders','getDriverOrders','getPickups','getPendingPurchases','approvePurchase','submitDriverReturn','uploadDriverPhoto']);
exports.handle=async(body,user,safe)=>{
  ...
  if(['getDriverOrders','getPickups','submitDriverReturn','uploadDriverPhoto'].includes(action)){
    driver(user);
    if(action==='uploadDriverPhoto')fail('Staging ยังไม่ได้ตั้ง Telegram ห้องทดสอบ จึงยังส่งรูปไม่ได้',503);
  ...
  if(action==='sendPayrollNotify')return {success:true,notification:'disabled_on_staging'};
