(() => {
  ...
  window.openMyProfile=async staffId=>{
    ...
    try{
      ...
      const form=node('form');const fields=[['name','ชื่อ / นามสกุล','text',80],['nickname','ชื่อเล่น','text',80],['age','อายุ','number',3],['address','ที่อยู่ปัจจุบัน','textarea',1000],['bank_account','เลขบัญชีธนาคาร','text',40]];
      for(const [key,label,type,max] of fields){const l=node('label',label),input=node(type==='textarea'?'textarea':'input');input.id='profile-'+key;input.name=key;l.htmlFor=input.id;if(type!=='textarea')input.type=type;else input.rows=3;input.value=data[key]??'';input.readOnly=readOnly;input.maxLength=max;if(key==='name')input.required=true;if(key==='age'){input.min=1;input.max=120;input.step=1;}if(key==='bank_account'||key==='age')input.inputMode='numeric';form.append(l,input);}
